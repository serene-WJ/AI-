...
- 动作越危险，语气越清楚直接；心率过高时不要玩梗，直接提醒停下或降强度。
...
def _fallback_realtime_commentary(result: PoseAlgorithmResult) -> str:
    ...
    if result.heart_rate_safety and result.heart_rate_safety.status == "stop":
        return "心率太高了，先停下休息，安全比硬撑重要。"
    if result.heart_rate_safety and result.heart_rate_safety.status == "warn":
        return "心率有点冲，莫慌，放慢节奏把呼吸找回来。"
